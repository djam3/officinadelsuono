          ...
          const AI_FEATURE_DEFS = [
            {
              ...
              icon: MessageSquare,
              name: 'Consulente AI AM3',
              subtitle: 'Chatbot di pre-vendita esperto DJ',
              description: 'Chatbot intelligente che guida i visitatori indecisi verso il kit o prodotto giusto, usando la voce di Amerigo certificato MAT Academy. Non fa assistenza generica: il suo unico scopo è capire cosa cerca il cliente e portarlo al checkout del prodotto più adatto.',
              costApi: '~€0,01-0,02 a conversazione',
              impact: 'Conversione +150-250% sul traffico che interagisce',
              priority: 5,
              implemented: false,
            },
            {
              ...
              icon: Target,
              name: 'Quiz Trova il Tuo Setup',
              subtitle: 'Consulenza guidata AI con lead capture',
              description: 'Quiz conversazionale AI di 3-4 domande adattive che capisce il livello, il budget e gli obiettivi del visitatore e consiglia il kit AM3 perfetto. Richiede email per inviare il consiglio personalizzato: ottimo lead magnet per costruire la mailing list.',
              costApi: '~€0,005-0,01 a quiz completato',
              impact: 'Conversione 8-15% sui 30 giorni successivi',
              priority: 5,
              implemented: false,
            },
            {
              ...
              icon: Sparkles,
              name: 'Descrizioni Prodotto Auto-SEO',
              subtitle: 'Generazione automatica schede prodotto ottimizzate',
              description: 'Quando carichi un nuovo prodotto inserisci solo nome e scheda tecnica: l\'AI genera titolo SEO, meta description, descrizione lunga firmata Amerigo MAT Academy, bullet point, FAQ e schema markup per Google. Lavora in background, non visibile al cliente.',
              costApi: '~€0,02 a prodotto (una tantum)',
              impact: '+traffico organico Google sul lungo periodo',
              priority: 4,
              implemented: false,
            },
            {
              ...
              icon: Star,
              name: 'Recensioni Aggregate AI',
              subtitle: 'Sintesi onesta delle opinioni dal web',
              description: 'L\'AI legge recensioni da YouTube, Reddit e forum DJ per ogni prodotto e genera una sintesi "cosa dicono i DJ di questo prodotto" con pro e contro. Aumenta il trust: il cliente trova tutto da te e non va a cercare su YouTube.',
              costApi: '~€0,05 a prodotto (aggiornamento trimestrale)',
              impact: 'Riduzione abbandono scheda prodotto, +trust',
              priority: 3,
              implemented: false,
            },
            {
              ...
              subtitle: 'Campagne personalizzate per cliente',
              description: 'L\'AI analizza lo storico acquisti e il comportamento di ogni cliente e genera email personalizzate automatiche: consigli accessori compatibili, novità coerenti con i gusti, offerte mirate. Tassi di apertura 40-50% vs 15-20% delle mass mail.',
              costApi: '~€0,01 a email generata',
              impact: 'Conversion rate email 3-5x vs mass mail',
              priority: 3,
              implemented: false,
            },
          ];
                ...
                {AI_FEATURE_DEFS.map(f => {
                  ...
                  return (
                    <motion.div
                      ...
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      className={`bg-zinc-900 border rounded-2xl p-6 transition-all duration-200 ${isEnabled ? 'border-brand-orange/30 shadow-[0_0_30px_rgba(242,125,38,0.06)]' : 'border-white/5'}`}
                    >
                      <div className="flex items-start gap-4">
                        {/* Icon */}
                        <div className={`p-3 rounded-xl shrink-0 ${isEnabled ? 'bg-brand-orange/15 text-brand-orange' : 'bg-zinc-800 text-zinc-400'}`}>
                          ...
                          <div className="flex items-start justify-between gap-4 flex-wrap">
                            <div>
                              <div className="flex items-center gap-3 flex-wrap mb-1">
                                ...
                                <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${isEnabled ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-zinc-800 text-zinc-500 border border-white/5'}`}>
                                  {isEnabled ? 'ATTIVA' : 'DISATTIVATA'}
                                </span>
                                {!f.implemented && (
                                  <span
                                    title="Infrastruttura pronta, attivazione nei prossimi mesi"
                                    className="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 cursor-help"
                                  >
                              ...
                              <div className="flex flex-wrap gap-4 text-xs">
                                ...
                                <span className="flex items-center gap-1.5 text-zinc-500">
                                  ...
                                  Impatto: <span className="text-zinc-300 font-bold">{f.impact}</span>
                                </span>
                                <span className="flex items-center gap-1.5 text-zinc-500">
                                  {'⭐'.repeat(f.priority)}
                                </span>
                            ...
                            <div className="flex flex-col items-end gap-3 shrink-0">
                              ...
                              <button
                                ...
                                role="switch"
                                className={`relative w-16 h-8 rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-brand-orange focus:ring-offset-2 focus:ring-offset-zinc-900 ${isEnabled ? 'bg-brand-orange' : 'bg-zinc-700'} ${isToggling ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'}`}
                              >
                                {isToggling ? (
                                  <Loader2 className="absolute inset-0 m-auto w-4 h-4 animate-spin text-white" />
                                ) : (
                                  <span className={`absolute top-1 w-6 h-6 bg-white rounded-full shadow transition-transform duration-200 ${isEnabled ? 'translate-x-9' : 'translate-x-1'}`} />
                                )}
                              </button>
              ...
              <AnimatePresence>
                {toggleToast && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 20 }}
                    className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 px-5 py-3 bg-zinc-800 border border-white/10 rounded-xl text-sm font-bold text-white shadow-2xl"
                  >
                    {toggleToast}
                  </motion.div>
                )}
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center justify-between mb-6">
                        ...
                        <button onClick={() => setConfigModalFeature(null)} className="text-zinc-500 hover:text-white transition-colors">
                          <X className="w-5 h-5" />
                        </button>
                      </div>
                      ...
                      <div className="space-y-4">
                          ...
                          <>
                            ...
                            <div>
                              ...
                              <textarea
                                ...
                                placeholder="Sei un esperto DJ certificato MAT Academy..."
                        ...
                        {configModalFeature === 'quiz_trova_setup' && (
                          <>
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-3">
                                <button onClick={() => setConfigDraft(p => ({ ...p, discountEnabled: !p.discountEnabled }))} className={`w-10 h-5 rounded-full transition-colors ${configDraft.discountEnabled ? 'bg-brand-orange' : 'bg-zinc-700'} relative`}>
                                  <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${configDraft.discountEnabled ? 'translate-x-5' : 'translate-x-0.5'}`} />
                                </button>
                        ...
                        {configModalFeature === 'descrizioni_seo_auto' && (
                          <>
                            <div>
                              <label className="block text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2">Tone of Voice (firma Amerigo)</label>
                        ...
                        {configModalFeature === 'recensioni_aggregate' && (
                          <>
                            ...
                            <div>
                              <label className="block text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2">Rating minimo prodotto (0 = tutti)</label>
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-3">
                                <button onClick={() => setConfigDraft(p => ({ ...p, showDisclaimer: !p.showDisclaimer }))} className={`w-10 h-5 rounded-full transition-colors ${configDraft.showDisclaimer ? 'bg-brand-orange' : 'bg-zinc-700'} relative`}>
                                  <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${configDraft.showDisclaimer ? 'translate-x-5' : 'translate-x-0.5'}`} />
                                </button>
                      ...
                      <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-white/5">
                        <button onClick={() => setConfigModalFeature(null)} className="px-4 py-2 text-sm text-zinc-400 hover:text-white transition-colors">Annulla</button>
                        <button
                          onClick={async () => {
                            ...
                            try {
                              ...
                              setToggleToast('Configurazione salvata');
                              setTimeout(() => setToggleToast(null), 3000);
                            } catch {
                              setToggleToast('Errore salvataggio configurazione');
                              setTimeout(() => setToggleToast(null), 3000);
                            } finally {
