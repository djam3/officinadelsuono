        ...
        {showAiSettings && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[60] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-zinc-900 border border-white/10 rounded-2xl p-8 max-w-lg w-full">
              ...
              <input type="password" value={manualApiKey} onChange={e => setManualApiKey(e.target.value)} className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-sm mb-4" placeholder="Groq API Key (console.groq.com)..." />
              <button onClick={() => { if (manualApiKey) localStorage.setItem('groq_api_key', manualApiKey); else localStorage.removeItem('groq_api_key'); setShowAiSettings(false); }} className="w-full py-3 bg-brand-orange text-white font-bold rounded-xl">Salva</button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
